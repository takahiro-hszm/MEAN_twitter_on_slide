...
r9.NicoScreen = {
	...
	draw: function(str){
		...
		//n.top_pos =  Math.floor( Math.random() * (parseInt(n.env.height) - 500) );
		n.top_pos =  30 + Math.floor( Math.random() * (parseInt(n.env.height)) );


		var end_left = (parseInt(n.env.width) + 200) * -1;
		...
		com_obj.delay(Math.floor(Math.random() * 1000)).animate({
			left: end_left
		}, n.env.speed,function(){
			var elm_id = "#" + cmid;
			$(elm_id).remove();
			this.top_pos = 10;
		});
